...
const credentials = {
  client: {
    id: `${config.variablesDef.CLIENT_ID}`,
    secret: `${config.variablesDef.CLIENT_SECRET}`
  },
  auth: {
    tokenHost: auth_token_route
  }
};
...
async function getAuthToken() {
  const tokenConfig = {
    username: `${config.variablesDef.AUTH_USERNAME}`,
    password: `${config.variablesDef.AUTH_PASSWORD}`,
    grant_type: 'password'
  };
...
async function getDisallow() {
  let fetchResult = await fetch(jsonapi_route)
  .then(res => res.json())
  .then(json => {
    let disallow = [];
    // get all the keys
  ...
  plugins: [
    ...
    `jquery`,
    {
      ...
      options: {
        ...
        secret: `${config.variablesDef.PREVIEW_SECRET}`,
        disallowedLinkTypes: [
