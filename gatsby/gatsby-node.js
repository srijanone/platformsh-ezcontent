    ...
    for(let inkey in nodes) {
      ...
      if(pageData.page) {
        ...
        actions.createPage({
          ...
          context: {
            drupal_host: backend_route,
            google_analytics: config.variablesDef.GOOGLE_ANALYTICS,
            environment: config.variablesDef.NODE_ENV,
            id: nodes[inkey].id
          },
        })
      }
...
exports.onCreatePage = async ({ page, actions }) => {
  ...
  // home page is generated by default
  // so override its context
  if(page.path === '/') {
    page.path = '/home'
  }
...
async function getPageData(pathAlias) {
  ...
  // when gatsby cloud lets get data directly from Drupal
  // else from the middelware
  if(config.variablesDef.NO_MIDDLEWARE && config.variablesDef.NO_MIDDLEWARE === "true") {
    let userParams = {
      request: {
        pathAlias: pathAlias,
      },
      ...
      oauth: {
        ...
        clientSecret: config.variablesDef.CLIENT_SECRET,
        clientId: config.variablesDef.CLIENT_ID
      },
      includedFields: [
        {
          type: 'article',
    ...
    let payloadObj = {};
    payloadObj["pathAlias"] = pathAlias;

    fetchPage = await postRequest(
      config.variablesDef.API_HOST + '/api/v1/getcontent',
      payloadObj
    );
